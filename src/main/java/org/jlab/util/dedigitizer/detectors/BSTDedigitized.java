 ...
 * @author ziegler
 */
public class BSTDedigitized extends ADegitizer {
    public BSTDedigitized() {
        this.setDetector("BST");
    }
    ...
    public void AppendHits(DataEvent event, PrintWriter pw, int eventNb, float startTime) {
        ...
        if (event.hasBank("BST::adc") == true) {
            ...
            for (int i = 0; i < rows; i++) {
               ...
               // pw.printf("%s\t %d\t %d\t %d\t %d\t %f\t %f\t %f\t\n", this.getDetector(), eventNb, sector[i], layer[i], component[i], 0.0, adcConv.SVTADCtoDAQ(ADC[i]), startTime);
            }
        }
    }
}
